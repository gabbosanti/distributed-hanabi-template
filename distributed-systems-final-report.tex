...
\usepackage{natbib}
\usepackage{graphicx}
\usepackage{cleveref} % this must beptance Criterion:} When the game ends \footnote{There are no cards left, or the number of misfires exceeds 3}, all clients receive a notification with the final game state and the outcome (win/loss).    
...
The following design is based on established distributed systems principles as described in the literature \cite{distributed-systems-book}.
...
{\scriptsize
  \begin{itemize}
    ...
    being produced should do. You should not focus on the particular problems, but exclusively on
...
%
  \begin{itemize}
    ...
    \item \textbf{Implementation}: constrain the entire phase of system realization, for instance by requiring the use of a specific programming language and/or a specific software tool
    %
    \begin{itemize}
...
Requirements are elicited from the problem domain and the intended use of the system.
They are categorized into functional, non-functional, and implementation requirements.

\subsubsection{Functional Requirements}

\begin{itemize}

  \item \textbf{FR1 - Game session creation :} \newline
  The system shall allow a user to create a new game session. \newline
  \emph{Acceptance criterion :} A user can create a session and receives a unique game ID.
  \item \textbf{FR2 - Join existing session :} \newline
  The system shall allow users to join an existing game session using a valid game ID. \newline
  \emph{Acceptance criterion: } Up to 5 players can successfully join the same session and are notified of other participants.
  \item \textbf{FR3 - Command validation :} \newline
  The system shall validate every player command according to the game rules before applying state changes. \newline
  \emph{Acceptance criterion: } If a player submits an invalid move, the system rejects it and the global state remains unchanged.
  \item \textbf{FR4 - State Broadcast :} \newline
  The system shall propagate every valid state update to all connected clients. \newline
  \emph{Acceptance criterion: } If a player submits a valid move, it is propagated through the system and the global state is changed.
...
\begin{itemize}
  ...
  \item \textbf{NFR1 :} The system shall allow players to connect and play togheter.
...
\begin{itemize}
  ...
  \item \textbf{IR1 :} The system shall allow players to connect and play togheter.
...
The system is composed of these architectural components.

\begin{itemize}
  \item are there \emph{infrastructural components} that need to be introduced? \emph{how many}?
  %
\begin{itemize}
    \item \textbf{Client :} Represent the user that is willing to play. The interaction is made by his browser
    \item \textbf{Server :} It's the core of the system : contains all of game logic, move validation and comunication with database
    \item \textbf{Load Balancer :} Redirect traffic from multiple client node to servers, also guarantee fault tolerance in case of server crash.
    \item \textbf{Redis Master :} It's the main node for storing data information about game and players
    \item \textbf{Redis Replica :} It's a replication of the current database, useful in case of crash of master node
\end{itemize}

\begin{figure}
    \centering
    \includegraphics[width=1.0\linewidth]{figures/hanabi-architecture.png}
