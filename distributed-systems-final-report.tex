...
Why is distribution needed?
  ...
  \begin{itemize}
    ...
    The players communicate through an unreliable, potentially lossy network. 
    \item computation speedup?
    \item resource sharing?
    The game state is the resource shared. Each player gets write privilege on his turn, so concurrent writes do not pose a problem. 
    \item fault tolerance?
    In order to mask faults from the players, the server process is replicated and by consensus mechanism the master is determined. This offers a level of fault tolerance at most the number of slaves that can successfully be promoted to master in case of failure. 
    \item other reasons?
  \end{itemize}
...
%
\begin{itemize}
  \item \textbf{1. Technology Stack} The system shall be implemented using Python for the server-side logic and React for the client-side interface.
  \begin{itemize}
    \item \textbf{Acceptance Criterion:} The server is implemented in Python, the client interface is built with React.
  \end{itemize}

  \item \textbf{2. Communication Protocol} The system shall use WebSockets for real-time communication between the server and clients.
  \begin{itemize}
  ...
  \item \textbf{3. Data Storage} The system shall use a Redis database to store game state and player information.
  \begin{itemize}
  ...
  \item \textbf{4. Containerization} The system shall be containerized using Docker to facilitate deployment and scalability.
  \begin{itemize}
